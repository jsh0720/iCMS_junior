    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			...
			//게시판 이미지 가져오기
			var wr_board = jindo.$$.getSingle("#wr_board").value;
	        ...
	        aFileList.push('	</li> ');
	        //한연재 추가 부분(alt텍스트 입력창 추가)
	        aFileList.push('	<li id="altimg'+nImageInfoCnt+'" class="imgLi">');
	        aFileList.push('	<input type="text" placeholder="이미지 설명 입력" id="img_alt_input'+nImageInfoCnt+'" class="img_alt_input"/>'); 
	        aFileList.push('	<input type="hidden" id="wr_board'+nImageInfoCnt+'" value="'+wr_board+'"/>'); 
	        aFileList.push('	</li> ');   
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/adm/se2_multi_uploader.do'; 	//upload URL
    	    	
    	//파일을 하나씩 보내고, 결과를 받음.
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		tempFile = htImageInfo['img'+j];
    		
    		//한연재 추가 부분, 알트 텍스트 입력한 값 가져오기 위해 추가
    		var img_alt = jindo.$$.getSingle("#img_alt_input" + j).value;
    		tempFile.sFileAlt = img_alt;
    		
    		//한연재 추가 부분, 에디터이미지 썸네일 제작을 위한 게시판 wr_board값 전달
    		var wr_board = jindo.$$.getSingle("#wr_board" + j).value;
    		tempFile.sBoardId = wr_board;
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
    	var oAjax = jindo.$Ajax(sUploadURL, {
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					...
					// 한연재 추가부분, 파일사이즈가 음수로 넘어올 때 오류메시지 표출
					else if(sResString.indexOf("FILESIZE_ERROR") > -1) {
						alert("파일 용량값이 올바르지 않아 오류가 발생하였습니다.\r\n관리자에게 문의주세요.");
					} else{
		...
		//한연재 추가 부분, 입력한 alt텍스트 헤더에 넣어 전송
		oAjax.header("file-alt", encodeURIComponent(tempFile.sFileAlt));
		//한연재 추가 부분, 게시판 wr_board 헤더에 넣어 전송
		oAjax.header("file-boardid", tempFile.sBoardId);
    ...
    /**
 	...
 	function delImage (sParentId){
 		...
 		//해당 li삭제
 		$Element(elLi).leave();
 		 //한연재 추가, alt텍스트 입력창 제거를 위해 추가
 		$Element($("alt" + sParentId)).leave();
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/adm/se2_single_uploader.do',	//샘플 URL입니다.
 	        sCallback : '/js/egovframework/diam/se2/quick_photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
