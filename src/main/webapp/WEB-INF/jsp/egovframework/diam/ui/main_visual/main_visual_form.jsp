    ...
    <div id="form_wrap">
    	<form id="fm" name="fm" method="post">
	        ...
	        <div class="page">
	            ...
	            <dl>
	                <dt>메인비주얼 제목<span class="required_value">*</span></dt>
	                <dd>
	                	<input type="text" name="dm_visual_alt" id="dm_visual_alt" maxlength="100" autocomplete="off">
	            ...
	            <dl>
	                ...
	                <dd id="image">
	                    <input type="file" name="multifile1" id="multifile1" accept=".jpg, .jpeg, .gif, .png">
	            ...
	            <dl>
	                <dt>링크</dt>
	                <dd>
	                	<input name="dm_visual_link" id="dm_visual_link" type="text" maxlength="255" autocomplete="off">
	                	<p class="noty">연결하고자 하는 외부페이지의  <b>http 프로토콜을 포함한 링크</b>를 입력해주세요. 최대 255자 작성가능합니다.</p>
	                    <p class="noty">단순 게시용 비주얼 이미지일 경우 링크를 입력하지 않고 등록해주세요.</p>
	                </dd>
	            </dl>
	            <dl>
	                <dt>링크 타겟<span class="required_value">*</span></dt>
	                <dd>
	                	<select id="dm_visual_link_type" name="dm_visual_link_type" class="easyui-combobox" panelHeight="auto"
	            ...
	            <dl>
	                ...
	                <dd>
	                    <input type="text" name="dm_order" id="dm_order" maxlength="2" onkeyup="setNumberPattern(this);" autocomplete="off">
		...
		$.ajax({
			...
			success : function (data) {
				if(data.result == "success") {
					...
					$("#dm_visual_alt").val(unescapeHtml(data.rows.dm_visual_alt));
					$("#dm_start_dt").val(data.rows.dm_start_dt);
			        $("#dm_end_dt").val(data.rows.dm_end_dt);
					$("#dm_visual_link").val(unescapeHtml(data.rows.dm_visual_link));
					$("#dm_visual_link_type").combobox('reload', '/adm/select_code.do?dm_code_group=1005&selected='+data.rows.dm_visual_link_type);
...
function fnSave() {
	if ($.trim($("#dm_visual_alt").val()) == "") {
		$.messager.alert("경고", "메인비주얼 제목을 입력하세요.", "warning");
		return;
	}
