...
<div class="bbs bbs_view bbs_<c:out value='${boardVO.dm_skin }'/>" id="bbs_<c:out value='${boardVO.dm_table }'/>">
	...
	<div class="bbs_viewing">
		...
		<c:if test="${boardVO.dm_use_link eq 1}">
			<c:if test="${writeVO.wr_link2 ne null && not empty writeVO.wr_link2}">
							...
							<c:forEach var="result" items="${fileList}" varStatus="status">
								<c:if test="${not empty oriFileList[status.index]}">
									<li>
										...
										<a href="/adm/get_write_file_download.do?file_name=<c:out value='${fileList[status.index]}'/>&ori_file_name=<c:out value='${oriFileList[status.index]}'/>&wr_id=<c:out value='${writeVO.wr_id}'/>&dm_table=<c:out value='${boardVO.dm_table}'/>">
